...
#variables to measure the elapsed time from the first to the last request
...
#open the socket to receive the requests
soc = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
soc.bind(('', PORT))
soc.listen()
print("Listening on port %s... Press CTRL+C to quit" % PORT)

#repeat until the CTRL+C signal is received
try:
	while True:
		#receive a request and answer yes if the url belongs to the set; no otherwise
...
#print the number of distinct urls visited by the clients and the elapsed time then exit
